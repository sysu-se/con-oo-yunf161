<script>
	...
	$: currentCellValue =
		$cursor.x === null || $cursor.y === null
			? null
			: $gameStore.getSudoku().getGrid()[$cursor.y][$cursor.x];
	$: canUndo = !$gamePaused && $gameStore.canUndo();
	$: canRedo = !$gamePaused && $gameStore.canRedo();
	$: canHint = !$keyboardDisabled && hintsAvailable && currentCellValue !== null;
